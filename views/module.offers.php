...
if(defined('OFFERS_PAGE') and isset($_REQUEST['slug'])) {
    ...
    if(!empty($recRow)) {
        ...
        $socialshare='<div class="share-social">
            ...
            <a class="twitter-share" target="blank" href="https://twitter.com/intent/tweet?text='.$recRow->title.' ?url='.BASE_URL.'offer/'.$recRow->slug.'" >
                <img src="'.BASE_URL.'template/web/assets/images/twitter.svg"><span>Share</span></a>
    ...
    $linkTarget = ($offer->linktype == 1)? ' target="_blank" ' : ''; 
            $linksrc    = ($offer->linktype != 1)? BASE_URL.$offer->linksrc : $offer->linksrc;
            ...
            $linkend    = '' ;
                 ...
                 $expired .='';
                //  $linkstart  = ($offer->linksrc!='')? '<a href="'.$linksrc.'" '.$linkTarget.'>' : '<a href="javascript:void(0);">';
                 $linkstart  = '<a href="'.BASE_URL.'offer/'.$offer->slug.'" '.$linkTarget.'>';
            $linkend    = ($offer->linksrc!='')? '</a>' : '</a>' ;
             }
            //  pr($expired);
            $resinndetail.='<div class="col-xl-4 col-md-6 col-sm-12">
